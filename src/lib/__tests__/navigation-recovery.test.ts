...
describe("navigation recovery", () => {
  it("recognizes Vite dynamic import failures", () => {
    expect(
      isDynamicImportLoadError(
        new TypeError(
          "Failed to fetch dynamically imported module: /assets/agenda.js",
        ),
      ),
    ).toBe(true);
    expect(isDynamicImportLoadError(new Error("Loading chunk 42 failed"))).toBe(
      true,
    );
    expect(
  ...
  it("normalizes unknown error values", () => {
    ...
    expect(navigationErrorMessage({ message: "route failed" })).toBe(
      "route failed",
    );
  });
  ...
  it("allows only one automatic reload during the cooldown", () => {
    ...
    expect(
      canReloadNavigation(String(now - NAVIGATION_RELOAD_COOLDOWN_MS), now),
    ).toBe(true);
