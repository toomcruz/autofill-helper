...
CREATE TABLE public.agenda_events (
  ...
  agenda_type TEXT NOT NULL CHECK (
    agenda_type IN ('exumacao', 'velorio_sepultamento', 'exumacao_pss')
  ),
  ...
  status TEXT NOT NULL DEFAULT 'agendado' CHECK (
    status IN ('agendado', 'confirmado', 'em_andamento', 'concluido', 'cancelado', 'pendente')
  ),
  notes TEXT,
...
CREATE INDEX agenda_events_user_date_idx
  ON public.agenda_events (user_id, event_date, agenda_type);

CREATE INDEX agenda_events_attendance_idx
  ON public.agenda_events (attendance_id)
  WHERE attendance_id IS NOT NULL;

CREATE UNIQUE INDEX agenda_events_attendance_type_unique
  ON public.agenda_events (attendance_id, agenda_type)
  WHERE attendance_id IS NOT NULL;
