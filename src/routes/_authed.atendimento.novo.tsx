...
function NewAttendance() {
  ...
  const visibleExtraFields = (
    proc?.extraFields?.filter(
      (field) =>
        !field.showWhen ||
        extras[field.showWhen.field] === field.showWhen.equals,
    ) ?? []
  ...
  function hasScheduleWithoutDate(): boolean {
    ...
    if (proc.key === "sepultamento" && extras.tem_velorio !== "SIM")
      return false;
    const scheduleKeys =
    ...
    return (
      scheduleKeys.some((key) => extras[key]?.trim()) &&
      !extras.data_agendada?.trim()
    );
  }
  ...
  async function createLinkedAgendaEvent(
    ...
    if (error)
      throw new Error(
        `Atendimento criado, mas a agenda não foi salva: ${error.message}`,
      );
  }
  ...
  async function submit() {
    ...
    if (hasScheduleWithoutDate()) {
      return toast.error(
        "Informe a data para adicionar este atendimento à agenda.",
      );
    }
      ...
      {step === "details" && proc && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Defina os detalhes do atendimento. Os campos de velório só
          ...
          <CardContent className="space-y-5">
            ...
            <ExtraFields
              fields={
                isSepultamento
                  ? visibleExtraFields.filter(
                      (field) => !TRIAGEM_SEPULTAMENTO_KEYS.has(field.name),
                    )
                  : visibleExtraFields
              }
              values={extras}
      ...
      {step === "upload" && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              A IA extrai os dados do atendimento e atualiza a agenda quando
              houver vínculo.
            </CardDescription>
          </CardHeader>
              ...
              <div className="rounded-md border bg-muted/30 p-3 text-sm flex items-start gap-2">
                ...
                <div>
                  <div className="font-medium">
                    Este atendimento será incluído na Agenda Geral
                  </div>
